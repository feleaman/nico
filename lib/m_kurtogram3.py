...
def Fast_Kurtogram(x, nlevel, Fs=1, opt1=None, opt2=None):
	...
	if opt1 == 1:
		...
		# print(type(Kwav[0][0]))
		# sys.exit()
		plt.imshow(Kwav_loc,aspect='auto',extent=(freq_w[0],freq_w[-1],Level_w[-1],Level_w[0]),interpolation='none')
...
def Fast_Kurtogram_filters(x, nlevel, Fs=1, opt1=None, opt2=None):
	# Fast_Kurtogram(x,nlevel,Fs)
	# Computes the fast kurtogram of signal x up to level 'nlevel'
	# Maximum number of decomposition levels is log2(length(x)), but it is 
	# recommed to stay by a factor 1/8 below this.
	...
	if opt1 == 1:
		...
		print('+++Width Freq: ', Fs/2.**(l1+1))
		print('+++Lowpass Freq: ', Fs*fi - (Fs/2.**(l1+1))/2.)
		lowpass = Fs*fi - (Fs/2.**(l1+1))/2.
		print('+++Highpass Freq: ', Fs*fi + (Fs/2.**(l1+1))/2.)
		highpass = Fs*fi + (Fs/2.**(l1+1))/2.
		# plt.colorbar()
		# plt.show()
	else:
		logging.error('stft-based is not implemented')
	
	return lowpass, highpass, maxima_kurtosis
...
def K_wpQ_local(x,h,g,h1,h2,h3,nlevel,opt,level):
	...
	if level > 1:
		...
		Ka1 = Ka1*np.ones(int(Long))
		Ka2 = Ka2*np.ones(int(Long))
		Ka3 = Ka3*np.ones(int(Long))
		Kd1 = Kd1*np.ones(int(Long))
		Kd2 = Kd2*np.ones(int(Long))
		Kd3 = Kd3*np.ones(int(Long))
...
def binary(i,k):
	...
	for l in np.arange(k,0,-1):
		a[int(k-l)] = np.fix(temp/2**l)
		temp = temp - a[int(k-l)]*2**l
	return a
...
if __name__ == "__main__":
	...
	# x = v1['v1']
	Fs = 1.e6
	# x = np.array([np.sin(i*dt*2*np.pi*frec) for i in range(n_points)])
	...
	root.destroy()
	x = load_signal(filepath, channel='AE_0')
	
	# x = x[0:int(len(x)/2)]
	# x = butter_highpass(x=x, fs=Fs, freq=20.e3, order=3)
	# x = x**2.0
	...
	nlevel= 5
	lp, hp = Fast_Kurtogram_filters(x, nlevel, Fs)
	print(lp)
	print(hp)