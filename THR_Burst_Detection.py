...
Opt_Input = {'interval':0, 'n_files':1, 'files':None, 'window_time':0.001, 'data_norm':None, 'plot':'OFF', 'save_name':'NAME'}
...
Opt_Input_thr = {'thr_mode':'fixed_value', 'thr_value':2.6}
...
def main(argv):
	...
	if config['save'] == 'ON':
		...
		out_file = 'features_' + filename1[0:len(filename1)-5] + '_thr_' + str(config['thr_value']) + '_wtime_' + str(config['window_time']) + '_80khzHP' +'.pkl'
...
def thr_burst_detector(x1, config, count=None):
	...
	if config['denois'] != 'OFF':
		...
		x1 = butter_highpass(x=x1, fs=config['fs'], freq=80.e3, order=3, warm_points=None)
	else:
...
def plot_burst_rev(fig, ax, nax, t, x1, config, t_burst_corr1, amp_burst_corr1, t_burst_corr1rev, amp_burst_corr1rev):
	...
	if thr == True:
		...
		ax[nax].axhline(threshold1, color='k')		
	ax[nax].plot(t_burst_corr1, amp_burst_corr1, 'ro')
	ax[nax].plot(t_burst_corr1rev, amp_burst_corr1rev, 'go') 	
	# print(amp_burst_corr1/signal_rms(x1))
	# a = input('aaa')
