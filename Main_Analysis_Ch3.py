...
def invoke_signal(config):
	...
	if extension == 'mat':
		...
		x = f_open_tdms(filename, 'AE_0')
		# x = f_open_tdms_2(filename)
		y = f_open_tdms(filename, 'AE_1')
		z = f_open_tdms(filename, 'AE_2')
	...
	# x = x[int(3.23665*config['fs']) : int(3.23732*config['fs'])]
	# print(4.68464 - 4.68393)
	n = len(x)
	n_points = n
	tr = n*dt
	...
	config_demod = {'analysis':False, 'mode':'butter', 'prefilter':['bandpass', [120052.63, 147315.], 3], 
	'rectification':'only_positives', 'dc_value':'without_dc', 'filter':['lowpass', 5000., 3]}
	...
	# config_denois = {'analysis':True, 'mode':'butter_lowpass', 'freq':75.}
	# config_denois = {'analysis':False, 'mode':'butter_bandpass', 'freq':[70.e3, 150.e3]}
	# config_denois = {'analysis':False, 'mode':'butter_bandpass', 'freq':[140.e3, 210.e3]}
	...
	if config_denois['analysis'] == True:
		...
		if config_denois['mode'] == 'butter_highpass':
			...
			y = butter_highpass(x=y, fs=config['fs'], freq=config_denois['freq'], order=3, warm_points=None)
			z = butter_highpass(x=z, fs=config['fs'], freq=config_denois['freq'], order=3, warm_points=None)
		...
		for element in config_analysis:
			if config_analysis[element] == True:
				...
				if element == 'WFM':
					...
					# plt.tight_layout()
					title = 'AE_0' + ' ' + element + ' ' + filename
					title = title.replace('_', '-')
					ax_wfm.set_title(title, fontsize=11)
					...
					# ax_wfm[1].plot(t, x)
					ax_wfm.set_xlabel('Dauer [s]', fontsize=12)
					...
					# ax_wfm.plot(t, y)		
					# ax_wfm.set_ylabel('Amplitude [mV]', fontsize=12)
					# title = 'AE_1' + ' ' + element + ' ' + filename
					# title = title.replace('_', '-')
					# ax_wfm.set_title(title, fontsize=11)
					# ax_wfm.set_xlabel('Dauer [s]', fontsize=12)
					
					
					figwfm, axwfm = plt.subplots(ncols=1, nrows=3, sharex=True)
					axwfm[0].plot(t, x)
					# axwfm[0].set_ylabel('Amplitude [mV]', fontsize=12)
					# axwfm[0].set_xlabel('Dauer [s]', fontsize=12)
					title = 'AE_0' + ' ' + element + ' ' + filename
					title = title.replace('_', '-')
					axwfm[0].set_title(title, fontsize=10)
					
					axwfm[0].tick_params(axis='both', labelsize=11)
					axwfm[1].tick_params(axis='both', labelsize=11)
					axwfm[2].tick_params(axis='both', labelsize=11)
					
					axwfm[1].plot(t, y)
					axwfm[1].set_ylabel('Amplitude [mV]', fontsize=12)
					# axwfm[1].set_xlabel('Dauer [s]', fontsize=12)
					title = 'AE_1' + ' ' + element + ' ' + filename
					title = title.replace('_', '-')
					axwfm[1].set_title(title, fontsize=10)
					
					axwfm[2].plot(t, z)
					# axwfm[2].set_ylabel('Amplitude [mV]', fontsize=12)
					axwfm[2].set_xlabel('Dauer [s]', fontsize=12)
					title = 'AE_2' + ' ' + element + ' ' + filename
					title = title.replace('_', '-')
					axwfm[2].set_title(title, fontsize=10)
					...
					ax_fft.set_title(title, fontsize=11)
