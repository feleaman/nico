...
def main(argv):
	...
	if config['mode'] == 'ann_test':		
		...
		#++++++++++++++++++++++++++++++ SAVING
		save_pickle('Scores_' + config['name'] + '.pkl', {'MSE_Valid':MSE_Valid, 'MSE_Test':MSE_Test, 'MSE_Train':MSE_Train, 'CRC_Valid':CRC_Valid, 'CRC_Test':CRC_Test, 'CRC_Train':CRC_Train})
		save_pickle('config_' + config['name'] + '.pkl', config)
		...
		Combi, Epoch_Combi = calculate_score_regression(Filepaths)
		...
		if opt != 'no':
			...
			for layer, alpha, n_pre, m_post, minibatches, epochs in zip(Layers, Alphas, N_Pres, M_Posts, MiniBatches, Epochs):
				if count >= 0:
				# print('++++++++++++++++...', layer)
					os.system('python M_Regression2.py --plot OFF --rs 0 --hidden_layers 0 ' + layer + ' --mode ann_validation --name Gen' + opt + '_CORR_Idx14_ANN_Idx_' + str(count) + ' --activation identity --alpha ' + alpha + ' --n_pre ' + n_pre + ' --m_post ' + m_post + ' --n_mov_avg 12 --train 0.6 --feature CORR' + ' --predict_node last --valid 0.2 --mini_batches ' + minibatches + ' --epochs ' + epochs + ' --mypath ' + mypath)
				count += 1
