		...
		for element in config_analysis:
			if config_analysis[element] == True:
				...
				if element == 'WFM':
					...
					figwfm, axwfm = plt.subplots(ncols=1, nrows=2, sharex=True)
					axwfm[0].plot(t, x)
					axwfm[0].set_ylabel('Amplitude [mV]', fontsize=12)
					# axwfm[0].set_xlabel('Dauer [s]', fontsize=12)
					title = 'AE_0' + ' ' + filename[:-5]
					title = title.replace('_', '-')
					axwfm[0].set_title(title, fontsize=11)
					...
					title = 'AE_1' + ' ' + filename[:-5]
					title = title.replace('_', '-')
					axwfm[1].set_title(title, fontsize=11)
					
					# axwfm[0].set_ylim(-600, 550)
					# axwfm[1].set_ylim(-200, 200)
