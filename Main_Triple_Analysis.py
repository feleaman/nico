...
def invoke_signal(config):
	...
	try:
		x1 = load_signal_2(filename, channel='AE_1')
		x2 = load_signal_2(filename, channel='AE_2')
		x3 = load_signal_2(filename, channel='AE_3')
	except:
	...
	if config['range'] != None:
		x1 = x1[int(config['range'][0]*config['fs']) : int(config['range'][1]*config['fs'])]
		x2 = x2[int(config['range'][0]*config['fs']) : int(config['range'][1]*config['fs'])]
		x3 = x3[int(config['range'][0]*config['fs']) : int(config['range'][1]*config['fs'])]
	...
	# x = 1000*x*1000./70.8 #37 dB schottland
	# x = x*1000./70.8
	# x = 1000*x*1000./141.25 #43 dB bochum laststufen
	...
	config_stft = {'segments':1000, 'window':'hanning', 'mode':'magnitude', 'log-scale':False, 'SPK':False, 'surface':False}	
		...
		for element in config_analysis:
			if config_analysis[element] == True:
				...
				if element == 'WFM':
					...
					fig, ax_wfm = plt.subplots(nrows=3, ncols=1, sharex=True)
					...
					# # title = 'AE-1'
					ax_wfm[0].set_title('AE-1 ' + filename[:-4], fontsize=13)
					ax_wfm[1].set_title('AE-2 ' + filename[:-4], fontsize=13)
					ax_wfm[2].set_title('AE-3 ' + filename[:-4], fontsize=13)
