...
#+++++++++++++++++++++++++++CONFIG++++++++++++++++++++++++++++++++++++++++++
Inputs = ['channel', 'fs', 'method', 'n_files', 'save', 'save_plot']
Opt_Input = {'files':None, 'window_time':0.001, 'overlap':0, 'data_norm':None, 'clf_files':None, 'clf_check':'OFF', 'plot':'ON', 'save_name':'NAME'}
Opt_Input_analysis = {'EMD':'OFF', 'denois':'OFF', 'med_kernel':3, 'processing':'OFF', 'demod_filter':None, 'demod_prefilter':None, 'demod_rect':None, 'demod_dc':None, 'diff':'OFF', 'freq_hp':0., 'order_hp':0, 'power2':None}
...
Opt_Input_win = {'rms_change':0.5, 'rms_change_mode':'fixed'}
...
def main(argv):
	...
	for k in range(config['n_files']):		
		...
		if filename1.find('80') != -1:
			label = label + '_' + str(80)
		else:
			label = label + '_' + str(40)
		print(label)
	...
	if config['save'] == 'ON':
		config['save_name'] = config['save_name'] + '_' + config['method']
		if config['method'] == 'THR':
			config['save_name'] = config['save_name'] + '_' + str(config['thr_value']) + '_' + config['thr_mode']
		elif config['method'] == 'EDG':
			config['save_name'] = config['save_name'] + '_' + str(config['thr_value']) + '_' + str(config['demod_filter']) + '_' + str(config['demod_prefilter'])
		elif config['method'] == 'WIN':
			config['save_name'] = config['save_name'] + '_' + str(config['rms_change']) + '_overlap_025_corr'
	...
	for zoom in zoom_list:
		if config['plot'] == 'ON':
			...
			ylimitsRAW = ylimits_zoom(zoom, config, XRAW)

			
			fig = [[], []]
			fig[0], ax = plt.subplots(nrows=config['n_files'], ncols=1, sharex=True, sharey=True)
			for i in range(config['n_files']):
				plot_burst_paper(fig[0], ax, i, t, X[i], config, T_Burst[i], A_Burst[i], thr=True, color=None, zoom=zoom, ylimits=ylimits, clf=CLFs[i])
...
def burst_detector(x1, config, count=None):
	...
	if config['denois'] != 'OFF':
		print('with denois')
		x1 = signal_denois(x=x1, denois=config['denois'], med_kernel=config['med_kernel'], fs=config['fs'], freq=config['freq_hp'], order=config['order_hp'], warm_points=None)
	else:
	...
	if config['method'] != None:
		...
		if (config['method'] == 'THR' or config['method'] == 'EDG'):
			...
			if config['rms_change_mode'] == 'factor_rms':
				config['rms_change'] = config['rms_change'] * signal_rms(x1)
						...
						if clf_1[k] == 0:
							...
							for time_burst in t_burst_corr1:
								...
								if str(time_burst) in vec:
									...
									next = t_burst_corr1[count_local+1]
									if str(next) in vec:
										print(len(t_burst_corr1))
										del t_burst_corr1[count_local:count_local+2]
										# t_burst_corr1.pop(count_local)
										# t_burst_corr1.pop(count_local+1)
										print(len(t_burst_corr1))
									else:
										t_burst_corr1.pop(count_local)
...
def read_parser(argv, Inputs, InputsOpt_Defaults):
	...
	config_input['freq_hp'] = float(config_input['freq_hp'])
...
def plot_burst(fig, ax, nax, t, x1, config, t_burst_corr1, amp_burst_corr1, thr=None, name=None, color=None, clf=None, zoom=None, ylimits=None):
	...
	name = 'Test Signal: '
	ax[nax].set_xlabel('Time [s]', fontsize=12)
	...
	ax[nax].set_title(name, fontsize=12)
...
def plot_burst_paper(fig, ax, nax, t, x1, config, t_burst_corr1, amp_burst_corr1, thr=None, name=None, color=None, clf=None, zoom=None, ylimits=None):
	...
	if zoom != None:
		ax[nax].set_xlim(zoom[0], zoom[1])
		ax[nax].set_ylim(ylimits[0]*1.1, ylimits[1]*1.1)
		# ax[nax].ticklabel_format(style='sci', scilimits=(-1, 1), axis='y')
	...
	if (config['method'] == 'THR' or config['method'] == 'DFP' or config['method'] == 'EDG'):
		if thr == True:
			threshold1 = read_threshold(config['thr_mode'], config['thr_value'], x1)
			ax[nax].axhline(threshold1, color='k')
		ax[nax].plot(t_burst_corr1, amp_burst_corr1, 'k*', markersize=8)
		# for i in range(len(t_burst_corr1)):
			# ax[nax].axvspan(xmin=t_burst_corr1[i], xmax=(t_burst_corr1[i]+config['window_time']), facecolor='r', alpha=0.5)
		...
		for i in range(len(t_burst_corr1)):
			# ax[nax].axvspan(xmin=t_burst_corr1[i], xmax=(t_burst_corr1[i]+config['window_time']), facecolor='r', alpha=0.5)
			ax[nax].plot(t_burst_corr1[i], amp_burst_corr1[i], 'k*', markersize=8)
	...
	if flag != -1:
		flag2 = config['filename'].find('80')
		if flag2 != -1:
			name = name + '1500 [RPM] / 80% Load'
			plotname = '_1500_80_'
		else:
			name = name + '1500 [RPM] / 40% Load'
			plotname = '_1500_40_'
	else:
		flag2 = config['filename'].find('80')
		if flag2 != -1:
			name = name + '1000 [RPM] / 80% Load'
			plotname = '_1000_80_'
		else:
			name = name + '1000 [RPM] / 40% Load'
			plotname = '_1000_40_'

	ax[nax].set_title(name, fontsize=12)
	# ax[nax].set_ylabel('Amplitude')
	if config['method'] == 'NN' or config['method'] == 'EDG':
		ax[nax].set_ylabel('Diff. Envelope [mV]', fontsize=12)
	elif config['method'] == 'DFP':
